...
#define V0_MAX                  65      /* 65*1000=65000, avoid uint16 timer overflow */
...
{
  uint16 tmp = (uint16)atoi(val);
  if (tmp == 0) tmp = 1;
  if (tmp > V0_MAX) tmp = V0_MAX;
  V0 = tmp;
}
/*********************************************************************************************
* ���ƣ�updateA0()
...
{ 
  ...
  if (0 == strcmp("night", ptag)){
    security_logic_set_night((uint8)val);
    ret = sprintf(p, "%u", (val != 0) ? 1 : 0);
    ZXBeeAdd("night", p);
  }
  if (0 == strcmp("arm", ptag)){
    security_logic_set_night((uint8)(val == 2));
    ret = sprintf(p, "%u", val);
    ZXBeeAdd("arm", p);
  }
...
void MyEventProcess( uint16 event )
{
  ...
  if (event & MY_REPORT_EVT) { 
    sensorUpdate();
    /* ??????�???��??�???��??�?�???��?????�???��??�? MY_REPORT_EVT */
    osal_start_timerEx(sapi_TaskID, MY_REPORT_EVT, (uint16)((uint32)V0*1000));
  }  
